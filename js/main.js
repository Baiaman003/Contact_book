    ...
    success:function(data){
      ...
      data.map(item=>{
        if(item.name === $('.who').val()){
          ...
          <li class="liMod">Имя: <span>${item.name}</span></li>
          <li class="liMod">Фамилия: <span>${item.surname}</span></li>
          <li class="liMod">Номер: <span>${item.number}</span></li>
          `);
      }
      });
      if(search===true){
